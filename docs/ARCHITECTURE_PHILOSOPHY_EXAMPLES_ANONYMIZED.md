...
> **實作範例：**
> 
> *   **情境**：原始資料是 `TechCorp-John`
> *   **傳統做法 (Bad)**：把 `TechCorp-John` 改成 `TechCorp` (原始人名遺失了)
> *   **本專案做法 (Good)**：
>     *   建立公司 ID `COM-EX001` (TechCorp)
>     *   建立聯絡人 ID `CON-EX001` (John)
>     *   建立關聯：`RAW-EX001` -> `COM-EX001` + `CON-EX001`
>     *   **結果**：資料被拆解並「歸位」，沒有任何字串被真正「刪除」。
...
> | 欄位 | 範例值 | 說明 |
> |---|---|---|
> | **Raw Identifier** | `RAW-202X0101-001` | 指向原始 CSV 第 1 行的 `(OldGroup)ConstructorCorp` |
> | **Stable Internal ID** | `COM-EX002` | 指向標準公司 `ConstructorCorp Ltd.` |
> | **Mapping Decision** | `manual_override` | 記錄這是人工審核決定的 |
...
## 整合與應用：投影至剛性系統 (Projection to Rigid Systems)

當資料需要離開清洗系統，進入外部的「剛性系統」（如 ERP、CRM）時，通常會面臨嚴格的欄位限制（例如：統編必須唯一、名稱長度限制）。這時我們採取「投影」策略：

> **實作範例：**
> *   **挑戰**：外部系統有嚴格的唯一性限制，強迫我們丟失「重複但有意義」的原始資料（例如：多個不同寫法的公司對應到同一個統編）。
> *   **本專案解法**：利用外部系統的「彈性欄位」（如 Tags、Notes、Custom Fields）來儲存我們的 **Traceability Context**。
> *   **結果**：在 V3.5 中，我們將原始的「備註」、「關係企業」投影為系統 **標籤 (Tags)**，確保即使在平面化的資料庫中，脈絡 (Context) 依然可見。
